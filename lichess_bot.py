...
def accept_challenge(session: requests.Session, challenge_id: str) -> bool:
    r = session.post(
        f"{LICHESS}/api/challenge/{challenge_id}/accept",
        timeout=30,
    )
    ...
    def handle_game_stream(self) -> None:
        ...
        try:
            ...
            if resp is not None and resp.status_code == 429:
                ...
                LOG.warning(
                    "game stream 429 for %s; open only one stream per game. Sleeping %.0fs",
                    self.game_id,
                    wait,
                )
                time.sleep(wait)
    ...
    while True:
        try:
                ...
                for event in _ndjson_lines(resp):
                    ...
                    if et == "challenge":
                        ...
                        if rated and not accept_rated:
                            ...
                            continue
                        LOG.info(
                            "incoming challenge %s from %s rated=%s",
                            ch_id,
                            challenger or "?",
                            rated,
                        )
            ...
            if resp is not None and resp.status_code == 429:
                wait = _retry_after_seconds(resp)
                LOG.warning(
                    "event stream rate limited (429); waiting %.0fs before reconnect",
                    wait,
                )
                time.sleep(wait)
                backoff = 5.0
